...
```{r}
g + geom_point(aes(color = y2, size = y2, fill = y2), shape=21) +
  geom_point(aes(x = x, y = y2, color = y2), size=4) +
  geom_point(aes(x = y2,y = x, color = x), size=4)+#we can co-plot a second variable with same colors
  scale_color_gradient(low="lightgreen", high = "darkgreen", name = "Color") + #change the color of the shapes
  scale_fill_gradient(low="lightgreen", high="darkgreen", name = "Fill") + #change the color of the fill
  geom_line(aes(x, y2, color = y), size = 1.5, alpha=.6) +
  geom_line(aes(x,y,color=x), size=1.5, alpha=.6)+
  geom_line(aes(y2,x, color=x), size=1.5, alpha=.6)+#add a transparent dashed line
